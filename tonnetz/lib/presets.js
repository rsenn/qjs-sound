...
function preset(name, chain, { style, seed, bpm, chordBars }, { x0, x1, y }) {
  ...
  return {
    ...
    state: {
      v: 2, fit: true, mods,
      seq: { pattern: makePattern(style, seed), edited: false, styleSel: style, chordBars, bpm },
...
export function stockPresets(area) {
  return [
    preset('Acid Liner', [
      ...
      ['delay', [0.6, 0.45, 0.3]],
    ], { style: 'acid', seed: 303, bpm: 128, chordBars: 2 }, area),
    preset('Piano Room', [
      ['fmp', [0.75, 0.5, 0.65]],
      ['delay', [0.55, 0.35, 0.3]],
      ['reverb', [0.4, 0.55]],
